...
def main():
    ...
    def preprocess_function(examples):
        # # |v1| context [SEP] question answer [SEP]
        # answers = examples[answer_name]
        # first_sentences = [[answer[j]['useful_context'] for j in range(len(answer))] for i, answer in enumerate(answers)]
        # second_sentences = [[question + ' ' + answers[i][j]['atext'] for j in range(len(answers[i]))] for i, question in enumerate(examples[question_name])]
        ...
        # question_char_pos = []
        # for i, second_sentence in enumerate(second_sentences):
        #     question_text = examples[question_name][i // len(answers[0])]
        #     question_text = question_text.strip()
        #     question_stt = second_sentence.find(question_text)
        ...
        #     q_stt = 0
        #     while tokenized_examples.sequence_ids(i)[q_stt] != 1:
        #         q_stt += 1
        #     while q_stt < len(offset) and offset[q_stt][0] < question_char_pos[i][0]:
        #         q_stt += 1
        #     q_end = len(offset) - 1
        #     while tokenized_examples.sequence_ids(i)[q_end] != 1:
        #         q_end -= 1
        ...
        #     # print(tokenizer.decode(tokenized_examples['input_ids'][i][q_stt:q_end]))

        # |v2| context question [SEP] answer [SEP]
        answers = examples[answer_name]
        first_sentences = [[answer[j]['useful_context'] + ' ' + examples[question_name][i] for j in range(len(answer))] for i, answer in enumerate(answers)]
        second_sentences = [[answer[j]['atext'] for j in range(len(answer))] for i, answer in enumerate(answers)]
        ...
        # Un-flatten
        rlts = {k: [v[i : i + len(answers[0])] for i in range(0, len(v), len(answers[0]))] for k, v in tokenized_examples.items()}
        labels = {
            'labels': [ra - 1 for ra in examples['ra']]
        }
        rlts = {**rlts, **labels}
        return rlts
